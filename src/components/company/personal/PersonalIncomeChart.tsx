...
export function PersonalIncomeChart() {
  ...
  const { data: chartData, isLoading } = useQuery({
    queryKey: ["personal-income-chart", user?.id],
    queryFn: async () => {
      ...
      // Group by month
      const monthlyData: { [key: string]: { month: string; recebido: number; pendente: number } } = {};
      ...
      transactions?.forEach((transaction) => {
        ...
        if (!monthlyData[monthKey]) {
          monthlyData[monthKey] = { month: monthLabel, recebido: 0, pendente: 0 };
        }
